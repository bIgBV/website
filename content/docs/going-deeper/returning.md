...
If you are using a version of rust after 1.26, you can use the language feature
[`impl Trait`]. This language feature will allow, for example:
...
The downside to this approach is that using a `Box` is still more flexible --
if you might return two different types of `Future`, then you must still return
`Box<Future<Item = F::Item, Error = F::Error>` instead of `impl Future<Item
= F::Item, Error = F::Error>`.  The good news however is that this case is
rare.
